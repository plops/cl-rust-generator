    ...
    let queue = queues.next().unwrap();
    {
        ...
        );
        {
            let f = std::fs::File::open(
                "/home/martin/stage/cl-rust-generator/examples/13_vulkano/code/src/trace.vert",
            )
            .expect("(format nil can't find ~a full-fn)");
            let mut v = vec![];
        ...
        {
            let f = std::fs::File::open(
                "/home/martin/stage/cl-rust-generator/examples/13_vulkano/code/src/trace.frag",
            )
            .expect("(format nil can't find ~a full-fn)");
            let mut v = vec![];
            f.read_to_end(&mut v).unwrap();
        ...
        {
            let f = std::fs::File::open(
                "/home/martin/stage/cl-rust-generator/examples/13_vulkano/code/src/trace2.frag",
            )
            .expect("(format nil can't find ~a full-fn)");
            let mut v = vec![];
            f.read_to_end(&mut v).unwrap();
