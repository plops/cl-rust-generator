...
// NOTE: there used to be a local `EncoderConfig' struct here.  It was never
// constructed and it shadowed `rav1e::prelude::EncoderConfig' (an explicit item
// wins over a glob import), which is why the rav1e one had to be referenced by
// its full path below.  The settings live in `crate::ServerConfig'.
...
impl Rav1eEncoder {
    pub fn new(
        ...
        let mut enc = EncoderConfig {
            ...
            min_quantizer: config.min_quantizer,
            speed_settings: SpeedSettings::from_preset(config.speed_preset as u8),
            tile_cols: config.tile_cols,
            tile_rows: config.tile_rows,

            // Apply latency-focused configuration from config parameters
            low_latency: !config.disable_low_latency,
            still_picture: !config.disable_still_picture,
            min_key_frame_interval: config.min_keyint,
            max_key_frame_interval: config.keyint,
            switch_frame_interval: config.switch_frame_interval,
            reservoir_frame_delay: Some(config.reservoir_frame_delay),
            error_resilient: !config.disable_error_resilient,
    ...
    pub async fn encode_frame(
        ...
        let yuv_image =
            core_utils::rgba_to_yuv420(screenshot, self.width as u32, self.height as u32)
        ...
        loop {
            ...
            loop {
                match self.ctx.receive_packet() {
                    Ok(packet) => {
                        ...
                        let packet_viewport_y = self
                            .viewport_y_queue
                            .pop_front()
                            .unwrap_or(current_viewport_y);
                        ...
                        trace!(
                            "[Server] Sending frame {} with viewport_y={}",
                            packet_count,
                            packet_viewport_y
                        );
                        updates.push(update);
                    ...
                    Err(rav1e::EncoderStatus::NeedMoreData) => {
                        ...
                        debug!(
                            "[Server] Encoder needs more data - this is normal for early frames"
                        );
                        break;
                    }
            ...
            if found_packet {
                // TODO: get pump_encoder_mode from config
                break;
            }
        ...
        info!(
            "[Server] No more packets, frame {} complete. Sent {} packets",
            self.frame_count, packet_count
        );
        self.frame_count += 1;
