...
use proto_def::graphical_proxy::{ClientEvent, ServerUpdate, TestFrameResult};
...
pub async fn handle_raw_image_data(
    ...
    println!(
        "[Server] Received test image: {}x{}, {} bytes",
        width,
        height,
        rgb_data.len()
    );
    ...
    let test_config = ServerConfig {
        quantizer: 150,    // Increased from default 100 to reduce file size
        min_quantizer: 80, // Set minimum quality floor
        speed_preset: 8,   // Balanced speed/quality
        tile_cols: 1,      // Minimal tiling to avoid errors
        tile_rows: 1,
        threads: 1,
        disable_low_latency: false,
        disable_still_picture: false, // Enable still picture mode for test frames
        ..Default::default()
    };
    ...
    let frame_result = encoder
        ...
        .await
        .map_err(|e| anyhow!("Test frame encoding failed: {:?}", e))?;
    ...
    let av1_data =
        if let Some(proto_def::graphical_proxy::server_update::Update::Frame(video_frame)) =
    ...
    println!(
        "[Server] Successfully encoded test frame: {} bytes",
        av1_data.len()
    );
