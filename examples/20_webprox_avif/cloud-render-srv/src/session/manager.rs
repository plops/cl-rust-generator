...
use proto_def::graphical_proxy::{ClientEvent, ServerUpdate, SpatialMetadata, VideoFrame};
use tonic::Status;

use crate::browser::{extract_spatial_metadata, CdpStream, ChromeRunner};
use crate::encoder::Rav1eEncoder;
...
impl SessionManager {
    ...
    pub async fn run(mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(Ok(first_event)) = self.client_stream.next().await {
            ...
            if let Some(proto_def::graphical_proxy::client_event::Event::TestImage(_)) =
                &first_event.event
            {
                ...
                while let Some(Ok(event)) = self.client_stream.next().await {
                    if let Some(proto_def::graphical_proxy::client_event::Event::TestImage(_)) =
                        &event.event
                    {
        ...
        let spatial_update = ServerUpdate {
            update: Some(
                proto_def::graphical_proxy::server_update::Update::SpatialData(SpatialMetadata {
                    ...
                    links: metadata
                        .links
                        .into_iter()
                        .map(|link| proto_def::graphical_proxy::LinkBox {
                            id: link.id,
                            url: link.url,
                            label: link.label,
                            x: link.x,
        ...
        if self
            .tx
            .send(Ok(spatial_update) as Result<ServerUpdate, Status>)
            .await
            .is_err()
        {
        ...
        // force_keyframes: default to simplified mode; the rest comes from the config
        let mut force_keyframes = true;
        let mut full_page_mode = self.config.full_page;
        let mut raw_rgb_mode = self.config.raw_rgb;
        let _pump_encoder_mode = self.config.pump_encoder;
        // Tracks exact Y for buffered frames:
        let _viewport_y_queue: std::collections::VecDeque<i32> = std::collections::VecDeque::new();
        // Track when last scroll was processed:
        ...
        loop {
                    ...
                    if let Some(client_event) = event.event {
                        match client_event {
                            ...
                            proto_def::graphical_proxy::client_event::Event::Scroll(scroll) => {
                                ...
                                info!(
                                    "[Server] Browser scroll execution took: {:?}",
                                    browser_scroll_time
                                );
                                ...
                                info!(
                                    "[Server] Total scroll processing time: {:?}",
                                    total_scroll_time
                                );
                                info!(
                                    "[Server] Browser scroll completed, viewport now at: {}",
                                    current_viewport_y
                                );
            ...
            let timestamp_text = if let Ok(result) = page
                .evaluate("document.getElementById('timestamp').textContent")
                .await
            {
            ...
            info!(
                "[Server] Screenshot captured: {}x{} in {:?}",
                screenshot.width(),
                screenshot.height(),
                screenshot_time
            );
            ...
            if raw_rgb_mode {
                ...
                let mut rgb_data = Vec::with_capacity(
                    screenshot.width() as usize * screenshot.height() as usize * 3,
                );
                for pixel in screenshot.as_raw().as_chunks::<4>().0 {
                ...
                let update = ServerUpdate {
                    update: Some(proto_def::graphical_proxy::server_update::Update::Frame(
                        VideoFrame {
                            av1_data: Vec::new(), // Empty when using raw RGB
                            is_keyframe: true,    // Always keyframe in raw mode
                            viewport_x: 0,
                            viewport_y: if full_page_mode {
                                0
                            } else {
                                current_viewport_y
                            },
                ...
                if self
                    .tx
                    .send(Ok(update) as Result<ServerUpdate, Status>)
                    .await
                    .is_err()
                {
            ...
            if let Some(max_frames) = self.config.max_frames {
                if frame_count >= max_frames {
                    info!(
                        "[Server] Reached maximum frame limit ({}), shutting down",
                        max_frames
                    );
                    break Ok(());
                }
            ...
            info!(
                "[Server] Sleeping for {}ms (configured loop delay)",
