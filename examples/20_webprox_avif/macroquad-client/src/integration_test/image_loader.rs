...
pub struct TestImageData {
    ...
    // Kept for the PNG dump of the received frame, which is not wired up yet
    // (see save_image_as_png below).
    #[allow(dead_code)]
...
pub fn load_test_image(path: &std::path::Path) -> Result<TestImageData> {
    ...
    let img =
        image::open(path).map_err(|e| anyhow!("Failed to load image {}: {}", path.display(), e))?;
...
// Debug helper for dumping the received frame; not wired into the test run yet.
#[allow(dead_code)]
