...
use proto_def::graphical_proxy::{
    client_event, remote_browser_client::RemoteBrowserClient, server_update, ClientEvent,
};

use crate::integration_test::{compare_images, load_test_image, report_results, TestOutcome};
...
pub async fn run_integration_test(
    ...
    let mut client = RemoteBrowserClient::connect(format!("http://{}", server_addr))
        .await
    ...
    while avif_data.is_none() && start_time.elapsed() < timeout_duration {
        match stream.next().await {
            Some(Ok(update)) => match update.update {
                Some(server_update::Update::TestFrame(test_frame)) => {
                    println!(
                        "[Client] Received test frame: {} bytes",
                        test_frame.av1_data.len()
                    );
    ...
    let decoded_image = decode_avif(&avif_data)
        .await
        .map_err(|e| anyhow!("AVIF decoding failed: {}", e))?;

    println!(
        "[Client] AVIF decoded successfully: {}x{}",
        decoded_image.width(),
    ...
    let outcome = if comparison.passed {
        ...
        TestOutcome::Failed(format!(
            ...
            comparison.different_pixels, comparison.total_pixels, comparison.max_difference
        ))
    };
...
async fn decode_raw_av1(av1_data: &[u8], config: &aom_decode::Config) -> Result<image::RgbaImage> {
    ...
    use aom_decode::{Decoder, RowsIters};
    use yuv::convert::RGBConvert;
    use yuv::YUV;

    let mut decoder = Decoder::new(config)?;
    let frame = decoder
        .decode_frame(av1_data)
