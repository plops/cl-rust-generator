...
use crate::state::{ClientImage, ClientState};
...
impl StreamHandler {
    pub fn handle_video_frame(
        ...
        info!(
            "Received video frame: {} bytes AV1, {} bytes RGB, keyframe: {}, viewport: ({},{})",
            frame.av1_data.len(),
            frame.raw_rgb_data.len(),
            frame.is_keyframe,
            frame.viewport_x,
            frame.viewport_y
        );
        ...
        if let Some(ref mut decoder) = lock.decoder {
            match decoder.decode_frame(&frame.av1_data) {
                Ok(Some(image_buffer)) => {
                    ...
                    debug!(
                        "First frame check: first_frame_received={}",
                        lock.first_frame_received
                    );
                    ...
                    info!(
                        "Successfully decoded AV1 frame: {}x{}, {} bytes",
                        width,
                        height,
                        frame.av1_data.len()
                    );
    ...
    fn handle_raw_rgb_frame(
        ...
        info!(
            "Processing raw RGB frame: {}x{}, {} bytes",
            frame.frame_width,
            frame.frame_height,
            frame.raw_rgb_data.len()
        );
        ...
        for chunk in frame.raw_rgb_data.as_chunks::<3>().0 {
    ...
    pub fn handle_spatial_metadata(
        ...
        info!(
            "Received spatial metadata: {} links, title: '{}'",
            metadata.links.len(),
            metadata.title
        );
        let mut lock = match state.lock() {
