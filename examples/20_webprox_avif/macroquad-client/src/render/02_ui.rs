...
pub fn draw_ui_and_frame(state: &Arc<Mutex<ClientState>>, texture: Option<&Texture2D>) {
    let (
        ...
        (
            lock.server_viewport_y,
            lock.local_scroll_y,
            lock.links.clone(),
            lock.doc_width,
            lock.doc_height,
            lock.page_title.clone(),
            lock.debug_y_offset,
            lock.verbose_coords,
        )
    };
    ...
    if let Some(tex) = texture {
        ...
        // Add controls help
        draw_text(
            "Controls: Mouse Wheel=Request Viewport",
            20.0,
            135.0,
            14.0,
            GREEN,
        ...
        draw_text(
            "Connecting to AV1 Remote Browser...",
        ...
        draw_text(
            "Make sure the server is running on [::1]:50051",
...
fn draw_link_overlays(
    ...
    for link in links {
        ...
        if verbose_coords {
            trace!(
                "[Client] Link: id={}, link_y={}, local_y={}, screen_y={}, render_offset_y={}",
                link.id,
                link.y,
                local_y,
                screen_y,
...
fn draw_status_overlay(
    ...
    // Draw page title
    draw_text(
        &format!("Page: {}", page_title.chars().take(40).collect::<String>()),
        20.0,
        35.0,
        16.0,
        GREEN,
    ...
    draw_text(
        &format!("Viewport: Server={} Local={}", server_y, local_y),
        20.0,
        85.0,
        16.0,
        YELLOW,
...
// Not wired up yet: link activation is still handled server side via CDP.
#[allow(dead_code)]
pub fn check_link_clicks(links: &[LinkBox], local_y: i32, debug_y_offset: i32) -> Option<String> {
    if is_mouse_button_pressed(MouseButton::Left) {
        ...
        for link in links {
            ...
            if screen_y > -link.height as f32
                && screen_y < screen_height()
                && mx >= screen_x
                && mx <= screen_x + link.width as f32
                && my >= screen_y
                && my <= screen_y + link.height as f32
            {
