...
pub fn handle_input_and_send_events(
    ...
    debug!(
        "[Client] Mouse wheel raw: wheel_y={}, last_wheel_y={}",
        mouse_wheel_y, *last_mouse_wheel_y
    );
    ...
    let scroll_input = if mouse_wheel_y != 0.0 {
        mouse_wheel_y
    } else if up_key {
        -1.0
    } else if down_key {
    ...
    if scroll_input != 0.0 {
        debug!(
            "[Client] Scroll triggered: input={}, source={}",
            scroll_input,
        ...
        *last_mouse_wheel_y = scroll_input;
        ...
        debug!(
            "[Client] Mouse Wheel: Requesting server viewport at absolute position {}",
            current_local_scroll_y
        );
        ...
        if let Err(e) = event_tx.send(scroll_event) {
            ...
            trace!(
                "[Client] ✓ Absolute scroll event sent to position {}",
                current_local_scroll_y
            );
        }
    }
