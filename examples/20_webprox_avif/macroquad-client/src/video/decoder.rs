...
/// What a decode attempt yields.  `Ok(None)` means "no frame ready yet".
type DecodedFrame = Result<Option<ImageBuffer<Rgba<u8>, Vec<u8>>>, Box<dyn std::error::Error>>;
...
impl Av1Decoder {
    ...
    pub fn decode_frame(&mut self, av1_data: &[u8]) -> DecodedFrame {
        println!(
            "[Client] Decoding {} bytes of AV1/AVIF data",
            av1_data.len()
        );
        ...
        match Avif::decode(av1_data, &config) {
            Ok(mut avif) => {
                ...
                match image {
                    Image::RGB8(img) => {
                        println!(
                            "[Client] Converting RGB8 to RGBA: {}x{}",
                            img.width(),
                            img.height()
                        );
                        ...
                        Ok(ImageBuffer::from_raw(self.width, self.height, rgba_data)
                            .ok_or("Failed to create RGBA image from RGB")?)
                    }
                    Image::RGBA8(img) => {
                        println!(
                            "[Client] Using RGBA8 directly: {}x{}",
                            img.width(),
                            img.height()
                        );
                        self.width = img.width() as u32;
                    ...
                    _other => Err("Unsupported image format".into()),
                }
            }
            Err(e) => {
    ...
    fn decode_raw_av1(
        ...
        let rgba_image = match frame.rows_iter()? {
            RowsIters::YuvPlanes8 {
                ...
                let mc = frame
                    .matrix_coefficients()
                    .unwrap_or(color::MatrixCoefficients::BT709);
    ...
    fn create_fallback_pattern(&self, av1_data: &[u8]) -> DecodedFrame {
        ...
        for y in 0..h {
            for x in 0..w {
                ...
                if distance < (pattern_size * pattern_size) as f32 {
                    ...
                    rgba_data[idx as usize + 1] =
                        (pattern_color[1] as f32 * intensity / 255.0) as u8;
                    rgba_data[idx as usize + 2] =
                        (pattern_color[2] as f32 * intensity / 255.0) as u8;
        ...
        println!(
            "[Client] Created fallback pattern for {} bytes",
            av1_data.len()
        );
        Ok(Some(
