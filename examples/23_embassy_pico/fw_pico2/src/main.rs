...
static PWM_CH: embassy_sync::channel::Channel<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    pico_link_proto::PwmCmd,
    8,
> = embassy_sync::channel::Channel::new();
static HSTX_CH: embassy_sync::channel::Channel<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    pico_link_proto::HstxCmd,
    4,
> = embassy_sync::channel::Channel::new();
static ADC_CH: embassy_sync::channel::Channel<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    pico_link_proto::AdcCmd,
    4,
> = embassy_sync::channel::Channel::new();
static CAP_CH: embassy_sync::channel::Channel<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    u8,
    4,
> = embassy_sync::channel::Channel::new();
static READ_CH: embassy_sync::channel::Channel<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    (u8, u16),
    4,
> = embassy_sync::channel::Channel::new();
static BLOCK_READY: embassy_sync::channel::Channel<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    (u8, u16),
    2,
> = embassy_sync::channel::Channel::new();
static BLOCK_BUF: embassy_sync::mutex::Mutex<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    [u16; 256],
> = embassy_sync::mutex::Mutex::new([0; 256]);
static BLOCK_META: embassy_sync::mutex::Mutex<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    (u8, u16),
> = embassy_sync::mutex::Mutex::new((0, 0));
static TEMP_VAL: embassy_sync::mutex::Mutex<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    u16,
> = embassy_sync::mutex::Mutex::new(0);
static CAP_VAL: embassy_sync::mutex::Mutex<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    u32,
> = embassy_sync::mutex::Mutex::new(0);
static CAP_PIO_VAL: embassy_sync::mutex::Mutex<
    embassy_sync::blocking_mutex::raw::ThreadModeRawMutex,
    u32,
> = embassy_sync::mutex::Mutex::new(0);
...
const SIN_Q16: [u16; 64] = [
    32768, 35979, 39160, 42279, 45307, 48214, 50972, 53555, 55938, 58097, 60013, 61666, 63041,
    64124, 64905, 65377, 65535, 65377, 64905, 64124, 63041, 61666, 60013, 58097, 55938, 53555,
    50972, 48214, 45307, 42279, 39160, 35979, 32768, 29556, 26375, 23256, 20228, 17321, 14563,
    11980, 9597, 7438, 5522, 3869, 2494, 1411, 630, 158, 0, 158, 630, 1411, 2494, 3869, 5522, 7438,
...
async fn adc_task(
    ...
    pin0: embassy_rp::Peri<'static, embassy_rp::peripherals::PIN_26>,
    pin1: embassy_rp::Peri<'static, embassy_rp::peripherals::PIN_27>,
    pin2: embassy_rp::Peri<'static, embassy_rp::peripherals::PIN_28>,
    pin_ts: embassy_rp::Peri<'static, embassy_rp::peripherals::ADC_TEMP_SENSOR>,
) {
    {
            ...
            while let Ok(req) = READ_CH.try_receive() {
                {
                    ...
                    let mut m: u16 = 0;
                    if ch == 0 {
                        m = if want < valid0 { want } else { valid0 };
                    } else {
                        if ch == 1 {
                            m = if want < valid1 { want } else { valid1 };
                        } else {
                            m = if want < valid2 { want } else { valid2 };
                        }
                    }
                    {
...
#[embassy_executor::task]
async fn pwm_task(
    p0: embassy_rp::pwm::Pwm<'static>,
    p1: embassy_rp::pwm::Pwm<'static>,
    p2: embassy_rp::pwm::Pwm<'static>,
    p3: embassy_rp::pwm::Pwm<'static>,
) {
    {
...
#[embassy_executor::task]
async fn cap_task(
    mut sm: embassy_rp::pio::StateMachine<'static, embassy_rp::peripherals::PIO0, 0>,
    p20: embassy_rp::Peri<'static, embassy_rp::peripherals::PIN_20>,
    p21: embassy_rp::Peri<'static, embassy_rp::peripherals::PIN_21>,
    p23: embassy_rp::Peri<'static, embassy_rp::peripherals::PIN_23>,
) {
    {
        let mut flexes: [embassy_rp::gpio::Flex<'static>; 3] =
            [Flex::new(p20), Flex::new(p21), Flex::new(p23)];
        let mut sel: u8 = 3;
        loop {
...
fn hstx_init() {
    pac::RESETS.reset().modify(|w| w.set_hstx(false));
    pac::RESETS.reset().modify(|w| w.set_dma(false));
    ...
    pac::CLOCKS
        .clk_hstx_ctrl()
        .modify(|w| w.set_auxsrc(pac::clocks::vals::ClkHstxCtrlAuxsrc::CLK_SYS));
    pac::CLOCKS.clk_hstx_ctrl().modify(|w| w.set_enable(true));
    pac::IO_BANK0.gpio(12).ctrl().modify(|w| w.set_funcsel(0));
    pac::PADS_BANK0.gpio(12).modify(|w| w.set_ie(false));
    pac::PADS_BANK0.gpio(12).modify(|w| w.set_od(false));
    pac::HSTX_CTRL.csr().modify(|w| w.set_en(false));
    pac::HSTX_CTRL.csr().modify(|w| w.set_expand_en(false));
    pac::HSTX_CTRL.csr().modify(|w| w.set_shift(31));
    pac::HSTX_CTRL.csr().modify(|w| w.set_n_shifts(0));
    pac::HSTX_CTRL.csr().modify(|w| w.set_clkdiv(7));
    pac::HSTX_CTRL.csr().modify(|w| w.set_clkphase(0));
    pac::HSTX_CTRL.bit(0).modify(|w| w.set_sel_p(0));
    pac::HSTX_CTRL.bit(0).modify(|w| w.set_sel_n(31));
    pac::HSTX_CTRL.bit(0).modify(|w| w.set_inv(false));
    pac::HSTX_CTRL.bit(0).modify(|w| w.set_clk(false));
    pac::HSTX_CTRL.csr().modify(|w| w.set_en(true))
}
fn hstx_dma_start(buf: &[u32; 256]) {
    {
        ...
        ch.write_addr()
            .write_value(pac::HSTX_FIFO.fifo().as_ptr() as u32);
        ch.trans_count()
            .modify(|w| w.set_mode(pac::dma::vals::TransCountMode::NORMAL));
        ch.trans_count().modify(|w| w.set_count(4294967295));
        ch.ctrl_trig().modify(|w| w.set_en(false));
        ch.ctrl_trig()
            .modify(|w| w.set_data_size(pac::dma::vals::DataSize::SIZE_WORD));
        ch.ctrl_trig().modify(|w| w.set_incr_read(true));
        ch.ctrl_trig().modify(|w| w.set_incr_write(false));
        ch.ctrl_trig().modify(|w| w.set_ring_size(10));
        ch.ctrl_trig().modify(|w| w.set_ring_sel(false));
        ch.ctrl_trig().modify(|w| w.set_chain_to(0));
        ch.ctrl_trig()
            .modify(|w| w.set_treq_sel(pac::dma::vals::TreqSel::HSTX));
        ch.ctrl_trig().modify(|w| w.set_irq_quiet(true));
        ch.ctrl_trig().modify(|w| w.set_en(true))
    }
}
fn hstx_rebuild(buf: &mut [u32; 256], freq_hz: u32, amp_tenth: u16, phase_deg: u16) {
    {
        let mut cyc: u32 = (freq_hz * 8192) / 15625000;
        cyc = if cyc < 1 {
            1
        } else {
            if 64 < cyc { 64 } else { cyc }
        };
        {
                    ...
                    for b in 0..32 {
                        {
                            let pos: u32 = w as u32 * 32 + b as u32;
                            {
                                ...
                                let duty: u32 = (SIN_Q16[((sph + poff) % 64) as usize] as u32
                                    * amp_tenth as u32)
                                    / 1000;
...
#[embassy_executor::task]
async fn hstx_task(buf: &'static mut [u32; 256]) {
    hstx_init();
    {
        let mut freq: u32 = 30;
        let mut amp: u16 = 800;
...
async fn send_status(tx: &mut embassy_rp::uart::BufferedUartTx, seq: u8) {
    {
        let tg = TEMP_VAL.lock().await;
        let cg = CAP_VAL.lock().await;
        {
            let msg = proto::StatusMsg {
                seq: seq,
                temp_c10: temp_raw_to_c10(*tg),
                cap: *cg,
                flags: 0,
            };
...
async fn send_block(tx: &mut embassy_rp::uart::BufferedUartTx, ch: u8, m: u16) {
    {
        let g = BLOCK_BUF.lock().await;
        {
            ...
            while off < m {
                {
                    let k: u16 = if m - off < 30 { m - off } else { 30 };
                    {
...
#[embassy_executor::task]
async fn uart_task(uart: embassy_rp::uart::BufferedUart) {
    {
        ...
        loop {
            {
                let r =
                    embassy_time::with_timeout(Duration::from_millis(100), rx.read_exact(&mut one))
                        .await;
                if let Ok(inner) = r {
                    if inner.is_ok() {
                                                ...
                                                if tag == proto::TAG_CAP_SELECT && n == 2 {
                                                    ...
                                                    if tag == proto::TAG_READ_BLOCK && n == 4 {
                                                        READ_CH
                                                            .send((
                                                                pl[1],
                                                                proto::get_u16_le(&pl[..4], 2),
                                                            ))
                                                            .await
                                                    } else {
                                                        defmt::warn!(
                                                            "uart: unknown tag={} len={}",
                                                            tag,
                                                            n
                                                        );
                                                    }
    ...
    async fn main(spawner: embassy_executor::Spawner) {
        {
            ...
            let prog = pio.common.load_program(&cap_prog.program);
            let mut cfg = PioConfig::default();
            let hbuf = HSTX_BUF_CELL.init([0; 256]);
            let mut led = Output::new(p.PIN_25, Level::Low);
            cfg.use_program(&prog, &[]);
            cfg.set_jmp_pin(&cap_pin);
            cfg.set_set_pins(&[&cap_pin]);
            cfg.set_in_pins(&[&cap_pin]);
            ...
            spawner.spawn(cap_task(pio.sm0, p.PIN_20, p.PIN_21, p.PIN_23).unwrap());
