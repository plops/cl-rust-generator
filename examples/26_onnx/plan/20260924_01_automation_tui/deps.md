...
|---|---|---|---|
| crossterm | crossterm-rs/crossterm | 0.29.0 (Prototyp: 0.28 — neueste genommen) | TUI: Raw-Mode, Events (Pfeile/`1`/`2`/`a`/`q`), Dashboard-Render, Terminal-Restore |
...
| toml | toml-rs/toml | 0.9.12 (händisches `Value`-Parsen, keine serde-Dep) | Regeln + Pan-Einstellungen aus `rules.toml` laden (`schema_version`-Check) |
